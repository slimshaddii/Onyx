...
class Launcher:
    ...
    Isolation works by temporarily moving the default RimWorld Config
    directory aside, writing the instance config in its place, launching
    the game, then restoring the default config after a short delay.
    """
    ...
    def launch(
        ...
        Steps performed:
          ...
          2. Ensure the executable is runnable (chmod on non-Windows).
          3. Create required instance subdirectories.
          4. Temporarily disable the default RimWorld Config (isolation).
          ...
          10. Start the process; restore config on failure or after 3 s.
        """
        exe = _resolve_exe(instance, self.rimworld_exe)
        if not os.path.isfile(exe):
            return LaunchResult(False,
                                f"RimWorld executable not found: {exe}")

        _ensure_executable(exe)
        _ensure_instance_dirs(instance)

        default_config_dir, temp_disabled = _isolate_default_config()
        ...
        prefs_path = instance.config_dir / 'Prefs.xml'
        if not prefs_path.exists():
            prefs_path.write_text(_DEFAULT_PREFS_XML, encoding='utf-8')

        if self.auto_backup and instance.has_saves:
            backup_folder(instance.saves_dir,
                          instance.path / '_backups',
                          self.backup_count)

        args = _build_launch_args(exe, instance, extra_args, log_to_instance)
        ...
        try:
            ...
            if temp_disabled:
                _restore_default_config(default_config_dir, temp_disabled)
            return LaunchResult(False, f"Failed to launch: {exc}")

    def is_running(self) -> bool:
        """Return True if a process was started and has not yet exited."""
    ...
    def get_playtime_minutes(self) -> int:
        """Return elapsed minutes since launch, or 0 if not launched."""
    ...
    def get_session_minutes_from_log(instance_path: Path) -> int:
        """
        Estimate session length from Player.log file timestamps.

        Uses file creation time (ctime) and last-modified time (mtime).
        Note: on Linux, ctime is the inode-change time, not creation time,
        so results may be approximate on that platform.
        ...
        try:
            ...
            return max(0, int((mtime - ctime).total_seconds() / 60))
        except OSError:
            return 0
    ...
    def _recover_orphaned_config(self) -> None:
        """
        Restore the default RimWorld Config if it was left disabled by a
        previous launcher crash before the 3-second restore timer fired.
        """
...
def _resolve_exe(instance: Instance, global_exe: str) -> str:
    """Return the effective RimWorld executable path for this instance."""
    override = getattr(instance, 'rimworld_exe_override', '')
...
def _ensure_instance_dirs(instance: Instance) -> None:
    ...
    for sub in ('Config', 'Saves', 'HugsLib', 'Scenarios', 'External'):
        (instance.path / sub).mkdir(parents=True, exist_ok=True)


def _isolate_default_config() -> tuple[Path, Optional[Path]]:
    """
    Move the default RimWorld Config aside so the game reads the instance
    config instead.

    Returns (default_config_dir, temp_path). temp_path is None if the
    move failed or there was nothing to move.
    """
    rw_data    = get_default_rw_data()
...
def _restore_default_config(default_path: Path,
                             temp_path: Path) -> None:
    """
    Move the temporarily disabled default Config back to its original location.

    Safe to call even if temp_path no longer exists (e.g. already restored).
    """
    try:
        if temp_path.exists():
...
def _build_launch_args(
    ...
    Merges instance launch_args and extra_args, deduplicating by flag name.
    Flags that take a separate value token (e.g. '-screen-width 1920') are
    joined into a single '=' form ('-screen-width=1920').
    """
    instance_abs = str(instance.path.resolve())
    args: list[str] = [exe, f'-savedatafolder={instance_abs}']

    if log_to_instance:
        log_abs = str((instance.path / 'Player.log').resolve())
        args.append(f'-logfile={log_abs}')

    seen: set[str] = {'-savedatafolder', '-logfile'}
            ...
            if arg.startswith('-'):
                ...
                if base not in seen:
                    ...
                    if (
                        ...
                        and not arg_list[idx + 1].startswith('-')
                    ):
