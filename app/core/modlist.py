...
def write_mods_config(config_path: Path, mod_ids: list[str],
                      ...
                      known_expansions: Optional[list[str]] = None) -> Path:
    """
    ...
    lines = [
        ...
        '  <activeMods>',
        *[f'    <li>{_xml_escape(mid)}</li>' for mid in ordered_mods],
        '  </activeMods>',
    ]
    ...
    if known_expansions:
        lines += [
            '  <knownExpansions>',
            *[f'    <li>{_xml_escape(exp)}</li>' for exp in known_expansions],
            '  </knownExpansions>',
        ]
...
def parse_rimsort_modlist(filepath: str) -> list[str]:
    """Parse a RimSort-format modlist text file and return a list of package IDs."""
    mod_ids: list[str] = []
    try:
...
def export_rimsort_modlist(filepath: str, mod_ids: list[str],
                           ...
                           version: str = '1.6.4630 rev467') -> None:
