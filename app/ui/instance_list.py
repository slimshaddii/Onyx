"""Instance list panel — Prism-style grouped card grid
...
class InstanceCard(QFrame):
    """Card widget representing a single instance
    in the grid."""
    ...
    def __init__(self, instance: Instance,
        ...
        stats = QLabel(
            f"📦{instance.mod_count}"
            f"  💾{instance.save_count}")
    ...
    def mouseMoveEvent(self, e: QMouseEvent):  # pylint: disable=invalid-name
        """Start a drag operation when the card is
        dragged with the left button."""
        if not (e.buttons()
...
class GroupHeader(QWidget):
    """Collapsible section header for an instance
    group."""
    ...
    def __init__(self, group_name: str,
        ...
        self._name_lbl.setFixedWidth(
            max(60, len(group_name) * 8))
        lo.addWidget(self._name_lbl)

        self._line = QFrame()
        self._line.setFrameShape(
            QFrame.Shape.HLine)
        self._line.setFrameShadow(
            QFrame.Shadow.Plain)
    ...
    def is_collapsed(self) -> bool:
        """Return True if this group is currently
        collapsed."""
        return self._collapsed

    def set_collapsed(self, val: bool):
        """Set the collapsed state and refresh the
        visual style."""
        self._collapsed = val
        self._refresh_style()

    def mousePressEvent(self, e: QMouseEvent):  # pylint: disable=invalid-name
        """Toggle collapse on left click; show
        context menu on right click."""
        if e.button() == Qt.MouseButton.LeftButton:
            self._collapsed = not self._collapsed
            self._refresh_style()
            self.toggle_requested.emit(
                self.group_name, self._collapsed)
        elif (e.button()
              == Qt.MouseButton.RightButton):
            self._show_context(
                self.mapToGlobal(e.pos()))
    ...
    def _show_context(self, global_pos):
        ...
        m.addAction(
            "✏️  Rename…",
            lambda: self.rename_requested.emit(
                self.group_name))
        m.addAction(
            "🗑  Delete group",
...
class GroupCardGrid(QWidget):
    """Card grid for all instances within a single
    group, with drag-drop support."""
    ...
    def set_drop_callback(self, cb):
        """Register a callback invoked when a card
        is dropped onto this grid."""
        self._on_drop_cb = cb

    def dragEnterEvent(self, e):  # pylint: disable=invalid-name
        """Accept drag if it carries an onyx instance
        payload."""
    ...
    def dragMoveEvent(self, e):  # pylint: disable=invalid-name
        """Keep accepting drag move while the payload
        is valid."""
        if e.mimeData().hasFormat(
                'application/x-onyx-instance'):
            e.acceptProposedAction()
    ...
    def dropEvent(self, e):  # pylint: disable=invalid-name
        """Handle a dropped instance card and invoke
        the drop callback."""
        if not e.mimeData().hasFormat(
                'application/x-onyx-instance'):
            e.ignore()
            return
        raw = e.mimeData().data(
            'application/x-onyx-instance')
        inst_path = bytes(raw).decode('utf-8')
        if self._on_drop_cb:
            self._on_drop_cb(
                inst_path, self.group_name)
        e.acceptProposedAction()

    def populate(self, instances: list[Instance],
                 cols: int,
                 selected: Instance | None,
                 on_click, on_double, on_context,
                 drop_callback=None):
        """Clear and rebuild all instance cards in a
        grid layout."""
        if drop_callback:
...
class InstanceGridPanel(QWidget):
    """Main panel showing all instances as a grouped
    card grid with search and sort."""
    ...
    SORTS = [
        ...
        ("Last Played",
         lambda x: x.last_played or '', True),
        ("Mods ↓",
         lambda x: x.mod_count, True),
        ("Saves ↓",
         lambda x: x.save_count, True),
        ("Newest",
         lambda x: x.created or '', True),
    ]
    ...
    def __init__(self, parent=None):
        ...
        self.sort_cb:      QComboBox | None = None
        self.scroll:       QScrollArea | None = None
        self.gc:           QWidget  | None = None
        self._content_lo:  QVBoxLayout | None = None
    ...
    def _build(self):
        ...
        self.group_filter.currentIndexChanged \
            .connect(self._rebuild)
        ...
        self._content_lo.setContentsMargins(
            0, 0, 0, 0)
    ...
    def set_instances(
        ...
        """Replace the instance list, refresh the
        group filter, and rebuild the grid."""
        ...
        self.group_filter.addItem("All Groups")
        groups = sorted(
            {i.group for i in insts if i.group}
            | self._empty_groups)
        for g in groups:
            self.group_filter.addItem(g)
        idx = self.group_filter.findText(
            current_group)
    ...
    def _rebuild_flat(
            self, instances: list[Instance],
            cols: int):
        """Populate the grid with all instances in a
        single ungrouped layout."""
        grid = GroupCardGrid('')
        grid.populate(
            instances, cols,
            self._selected_instance,
    ...
    def _rebuild_grouped(
        ...
        for group_name in all_group_names:
            group_instances = grouped.get(
                group_name, [])
            is_collapsed = self._collapsed.get(
                group_name, False)

            hdr = GroupHeader(
                group_name, is_collapsed,
                len(group_instances))
            hdr.toggle_requested.connect(
                self._on_group_toggle)
            hdr.rename_requested.connect(
                self._on_group_rename)
            hdr.delete_requested.connect(
                self._on_group_delete)
            self._content_lo.insertWidget(
                insert_pos, hdr)
            insert_pos += 1
        ...
        if (ungrouped
                ...
                in self._collapsed):
            is_collapsed = self._collapsed.get(
                _UNGROUPED_KEY, False)

            hdr = GroupHeader(
                "Ungrouped", is_collapsed,
                len(ungrouped))
            hdr.toggle_requested.connect(
                self._on_group_toggle)
            self._content_lo.insertWidget(
    ...
    def _on_group_toggle(self, group_name: str,
        ...
        for i in range(
                self._content_lo.count() - 1):
            w = self._content_lo.itemAt(
                i).widget()
            if (isinstance(w, GroupHeader)
                    and w.group_name
                    == group_name):
                grid_item = (
                    self._content_lo.itemAt(i + 1))
                if grid_item and grid_item.widget():
                    grid_item.widget().setVisible(
                        not is_collapsed)
                break
    ...
    def _on_group_rename(self, old_name: str):
        ...
        if old_name in self._collapsed:
            self._collapsed[new_name] = (
                self._collapsed.pop(old_name))
    ...
    def _on_group_delete(self, group_name: str):
        ...
        msg = (
            ...
            f"{len(affected)} instance(s) will "
            f"be ungrouped.")
    ...
    def _on_click(self, inst: Instance):
        ...
        for card in self._cards:
            card.set_selected(
                card.instance is inst)
        self.instance_selected.emit(inst)

    def _on_card_dropped(self, inst_path: str,
                         target_group: str):
        inst = next(
            (i for i in self.instances
             if str(i.path) == inst_path),
            None)
        if (inst is None
                or inst.group == target_group):
            return
        self._move_inst_to_group(
            inst, target_group)
    ...
    def _show_context(self, inst: Instance, pos):
        ...
        m.addAction(
            "✏️  Edit…",
            lambda: self.edit_requested.emit(inst))
        m.addAction(
            "🎨  Change Icon",
            lambda: self._change_icon(inst))
        m.addAction(
            "✏️  Rename",
            lambda: self._rename(inst))
        m.addSeparator()

        all_groups = sorted(
            {i.group for i in self.instances
             if i.group}
            | self._empty_groups)
        ...
        m.addAction(
            "📁  Open Folder",
            lambda:
                self.folder_requested.emit(inst))
        m.addAction(
            "📤  Export Modlist",
            lambda:
        ...
        m.addAction(
            "📋  Copy Instance",
            lambda:
                self.copy_requested.emit(inst))
        m.addSeparator()
        m.addAction(
            "🗑  Delete",
            lambda:
                self.delete_requested.emit(inst))
        m.exec(pos)
    ...
    def _move_inst_to_group(
            self, inst: Instance, group: str):
    ...
    def _set_color(self, inst: Instance,
                   color: str):
    ...
    def resizeEvent(self, e):  # pylint: disable=invalid-name
        """Rebuild the grid only when the column
        count changes."""
        super().resizeEvent(e)
        vw = self.scroll.viewport().width()
        vw = vw if vw > 50 else 350
