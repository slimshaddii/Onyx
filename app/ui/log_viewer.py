...
class LogViewerDialog(QDialog):
    ...
    def _build_ui(self):
        ...
        self.filter_combo.addItems(
            ["All", "Errors Only", "Warnings Only", "Info Only"])
    ...
    def _run_analysis(self):
        ...
        if not issues:
            ...
            for issue in issues:
                ...
                item.setForeground(color_map.get(issue.severity, default_color))
                self.issues_list.addItem(item)
    ...
    def _generate_startup_analysis(self):
        ...
        html = f"<h2 style='color:{colors['accent']};'>Startup Analysis</h2>"
        ...
        if analysis.phases:
            html += (f"<hr><h3 style='color:{colors['accent']};'>"
                     f"Startup Phases</h3>")
            html += "<table width='100%' cellspacing='4'>"
            html += (f"<tr>"
                     f"<th align='left' style='color:{colors['text_dim']};'>Phase</th>"
                     f"<th align='right' style='color:{colors['text_dim']};'>Duration</th>"
                     f"<th align='left' style='color:{colors['text_dim']};'>Impact</th>"
                     f"</tr>")
        ...
        if analysis.memory_stats:
            ...
            for stat in sorted(
                ...
                html += (
                    f"<tr>"
                    f"<td style='color:{colors['text']};'>{stat.name}</td>"
                    f"<td align='right' style='color:{color};'>"
                    f"<b>{display}</b></td>"
                    f"</tr>")
        ...
        if 'performanceoptimizer' in log_text.lower():
            html += (f"<li style='color:{colors['success']};'>"
    ...
    def _generate_summary(self, issues: list[LogIssue]):
        ...
        colors = _c()
        html   = f"""
        <h2 style='color: {colors['accent']}'>Log Analysis Summary</h2>
        <p>Total log lines: <b>{total}</b></p>
        <p style='color: {colors['error']}'>Errors: <b>{errors}</b></p>
        <p style='color: {colors['warning']}'>Warnings: <b>{warnings}</b></p>
        <hr>
        ...
        if issues:
            for issue in issues:
                icon  = {'error': '🔴', 'warning': '🟡', 'info': 'ℹ️'}.get(
                    issue.severity, '')
                html += (f"<p>{icon} <b>{issue.title}</b> "
                         f"(×{issue.count}) — {issue.suggestion}</p>")
        else:
            html += (f"<p style='color: {colors['success']}'>"
