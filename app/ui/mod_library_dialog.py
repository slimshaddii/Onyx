...
class ModLibraryDialog(QDialog):
    ...
    def _load(self):
        ...
        for d in sorted(self.mods_dir.iterdir()):
            ...
            it.setData(Qt.ItemDataRole.UserRole, len(self._mods) - 1)
    ...
    def _check_updates_async(self):
        ...
        def _run():
            try:
                ...
                mod_paths  = {m[0]: str(m[2]) for m in self._mods
                              if m[0].isdigit()}
    ...
    def _apply_filter(self):
        ...
        for idx, (wid, name, path, info, mtime) in enumerate(self._mods):
            if q and q not in name.lower():
                continue
            if filter_key == 'update' and wid not in self._has_updates:
                continue
            if filter_key == 'workshop' and not (wid and wid.isdigit()):
                continue
            if filter_key == 'local' and (wid and wid.isdigit()):
                continue
        ...
        if sort_key == 'name_asc':
            ...
            visible.sort(key=lambda x: x[2].lower(), reverse=True)
        ...
        update_count = sum(1 for e in visible if e[1] in self._has_updates)
        suffix = (f"  •  {update_count} update{'s' if update_count != 1 else ''}"
    ...
    def _on_select(self, row: int):
        ...
        if preview_path and preview_path.exists():
            ...
            if not pm.isNull():
                w = max(self._preview_lbl.width() - 4, 60)
                h = max(self._preview_lbl.height() - 4, 80)
        ...
        self._redl_btn.setEnabled(
            bool(wid and wid.isdigit() and self.download_manager))
        self._del_btn.setEnabled(True)

    def _on_selection_changed(self):
        """Update button states based on current selection count."""
        ...
        all_have_ws = all(
            ...
            and it.data(Qt.ItemDataRole.UserRole) < len(self._mods))
        self._redl_btn.setEnabled(
    ...
    def _delete(self):
        ...
        if len(to_delete) == 1:
            msg = (f"Permanently delete '{to_delete[0][1]}'?\n\n"
                   f"{to_delete[0][2]}\n\nCannot undo.")
        else:
            names = '\n'.join(f"  • {n}" for _, n, _ in to_delete[:5])
            extra = (f"\n  ... and {len(to_delete)-5} more"
                     if len(to_delete) > 5 else '')
            msg   = (f"Permanently delete {len(to_delete)} mods?\n\n"
                     f"{names}{extra}\n\nCannot undo.")
