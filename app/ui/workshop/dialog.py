...
from app.core.mod_linker import (
    link_mod_to_game, delete_downloaded_mod,
)
from app.core.rimworld import ModInfo
from app.core.steam_integration import (
    DownloadMethod, subscribe_via_steam,
    open_workshop_page,
)
...
class WorkshopBrowserDialog(QDialog):
    """Steam Workshop browser with optional WebEngine
    integration."""

    def __init__(self, _parent,
                 steamcmd: SteamCMDManager,
                 rw=None, _api_key='',
                 installed_workshop_ids=None,
                 download_method=(
                     DownloadMethod.STEAMCMD),
                 _is_steam_copy=False,
                 settings=None):
        super().__init__(None)
        self.steamcmd = steamcmd
        self.rw = rw
        self.installed_ids = set(
            installed_workshop_ids or set())
        ...
        self._dlq = DownloadQueue(
            ...
            max_concurrent=3,
            username=self._settings.get(
                'steamcmd_username', ''))

        from app.ui.modeditor.download_manager import DownloadManagerWindow  # pylint: disable=import-outside-toplevel
        self._dl_manager_window = (
            DownloadManagerWindow(self._dlq, self))
        self._dlq.job_started.connect(
            self._on_started)
        self._dlq.job_progress.connect(
            self._on_progress)
        self._dlq.job_finished.connect(
            self._on_finished)
        ...
        if can:
            try:
                ...
                # WebEngine init can fail in many
                # ways — fall back to manual mode
                print(
                    f"[Workshop] WebEngine "
                    f"failed: {e}")
                can = False
        if not can:
    ...
    def _build_fallback(self):
        ...
        msg = QLabel(
            ...
            f"{WE_ERROR or 'pip install '
               'PyQt6-WebEngine'}\n"
            "Enter a mod ID in the address bar.")
        msg.setStyleSheet(
            "padding:24px;color:#aaa;")
    ...
    def _build_with_browser(self):
        ...
        self._page.onyx_download.connect(
            self._download)
        ...
        self._browser.loadStarted.connect(
            lambda: self.status_lbl.setText(
                "Loading…"))
        self._browser.loadFinished.connect(
            self._on_loaded)
    ...
    def _make_nav(self) -> QWidget:
        ...
        self.addr.setPlaceholderText(
            "URL, mod ID, or search…")
        ...
        dl_mgr_btn.setStyleSheet(
            "font-size:10px;padding:2px 8px;")
        dl_mgr_btn.setToolTip(
            "Open Download Manager")
        dl_mgr_btn.clicked.connect(
        ...
        dl_coll_btn = QPushButton(
            "Get Collection")
        dl_coll_btn.setFixedHeight(26)
        dl_coll_btn.setStyleSheet(
            "font-size:10px;padding:2px 8px;")
        dl_coll_btn.setToolTip(
            "Download all mods from a Steam "
            "Workshop collection.\n"
            "Paste the collection URL or ID "
            "in the address bar first.")
        dl_coll_btn.clicked.connect(
        ...
        self.method_cb.setCurrentIndex(
            1 if self.download_method
            == DownloadMethod.STEAM_NATIVE
            else 0)
    ...
    def _inject(self):
        if (not self._browser
                or not self._browser.page()):
            return
        self._inject_ver += 1
        js = INJECT_JS.replace(
            '__INSTALLED_IDS__',
            json.dumps(
                list(self.installed_ids)))
        js = js.replace(
            '__VERSION__',
            str(self._inject_ver))
        self._browser.page().runJavaScript(js)

    # ── Download ─────────────────────────────────

    def _download(self, mod_id: str):
        if (self.method_cb.currentData()
                == 'steam_native'):
            if subscribe_via_steam(mod_id):
                QMessageBox.information(
                    self, "Steam",
                    f"Opened {mod_id} in Steam.")
            else:
        ...
        self._dlq.enqueue(
            mod_id, f"Item {mod_id}")

    def _open_download_manager(self):
        """Open the persistent download manager
        window."""
    ...
    def _download_collection(self):
        ...
        self.status_lbl.setText(
            f"Fetching collection {coll_id}...")
        QApplication.processEvents()

        mod_ids, error = (
            self._fetch_collection(coll_id))
        if error:
            QMessageBox.warning(
        ...
        self.status_lbl.setText(
            f"Queued {len(to_download)} "
            f"mod(s) from collection")

    @staticmethod
    def _fetch_collection(
            collection_id: str,
    ) -> tuple[list[str], str]:
        """Fetch mod IDs from a Steam Workshop
        collection.
        ...
        try:
            ...
            # Network call — heterogeneous
            # failures from requests/json
    ...
    def closeEvent(self, e):  # pylint: disable=invalid-name
        """Clean up signal connections and browser
        on close."""
        try:
            self._dlq.job_started.disconnect(
                self._on_started)
            self._dlq.job_progress.disconnect(
                self._on_progress)
            self._dlq.job_finished.disconnect(
                self._on_finished)
        except (RuntimeError, TypeError):
            # Qt signal disconnect can raise
        ...
        if self._browser:
            try:
                ...
                # WebEngine cleanup can raise
                # heterogeneous Qt errors
                pass
        super().closeEvent(e)
