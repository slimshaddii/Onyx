"""Library browser — add, delete, or redownload
mods from the global pool."""
...
class LibraryDialog(QDialog):
    """Browse all installed mods and pick ones to
    add to an instance."""

    def __init__(
            self, parent,
            all_mods: dict[str, ModInfo],
            instance_mod_ids: set[str],
            game_version: str = '',
    ...
    def _build(self):
        ...
        lo.addWidget(QLabel(
            "<b>Mod Library</b> — Select mods "
            "to add to this instance"))

        hint = QLabel(
            "These mods are installed on your "
            "system but not yet part of this "
            "instance. Select mods and click "
            "'Add to Instance'.\n"
            "Right-click a mod to delete or "
            "redownload it.")
        ...
        self.mod_list.itemSelectionChanged \
            .connect(self._on_sel_changed)
    ...
    def _load(self):
        ...
        self.count_label.setText(
            f"{count} mods available in library")
    ...
    def _filter(self):
        ...
        for i in range(self.mod_list.count()):
            ...
            item.setHidden(
                q not in item.text().lower())

    def _on_sel_changed(self):
        n = len(
            self.mod_list.selectedItems())
    ...
    def _ctx_menu(self, pos):
        ...
        m.exec(
            self.mod_list.mapToGlobal(pos))

    def _extra_mod_paths(self) -> list[str]:
        """Build extra mod scan paths from
        settings."""
    ...
    def _delete_mod(self, mid: str):
        ...
        for i in range(self.mod_list.count()):
            if (self.mod_list.item(i)
                    and self.mod_list.item(i)
                    .data(
                        Qt.ItemDataRole
                        .UserRole) == mid):
        ...
        visible = sum(
            1 for i in range(
                self.mod_list.count())
            if not self.mod_list.item(
                i).isHidden())
        self.count_label.setText(
            f"{visible} mods available "
            f"in library")

        QMessageBox.information(
            self, "Deleted",
            f"'{info.name}' was deleted.")
    ...
    def _redownload_mod(self, mid: str):
        ...
        dlg.setWindowTitle(
            f"Redownloading — {info.name}")
        dlg.downloads_complete.connect(
            lambda results:
                self._on_redownload_done(
                    results, info.name))
        dlg.exec()

    def _on_redownload_done(
            self, results: list,
            mod_name: str):
        ok = sum(
            1 for _, s, _ in results if s)
        if ok:
            QMessageBox.information(
                self, "Redownload Complete",
                f"'{mod_name}' was redownloaded"
                f" successfully.")
            if self.rw:
                self.all_mods = (
                    self.rw.get_installed_mods(
