"""Item creation and badge logic for the mod
editor."""
...
def _get_color(key: str) -> str:
    return get_colors(
        AppSettings.instance().theme)[key]
...
class ItemBuilder:
    ...
    def _badge_color(self, badges: list) -> str:
        ...
        worst = min(
            badges,
            key=lambda b:
                _SEVERITY_RANK.get(b[2], 99))
        return worst[1]

    def _make_item(
            self, label: str, mid: str,
    ...
    def _mk_active(
        ...
        if not self.all_mods.get(mid):
            ...
            tip = "Newly added to this instance"
        else:
        ...
        self.active.addItem(
            self._make_item(
                label, mid, color, tip, is_new))

    def _batch_load_active(
            self, mod_ids: list):
        """Load all active mods without per-item
        badge computation, then refresh."""
    ...
    def _refresh_badges(self):
        """Recompute badges for every item in
        the active list in one pass.

        Updates color, tooltip, and is_new on
        each ModItem directly, then emits
        ...
        for i in range(self.active.count()):
            ...
            mid = it.mid
            is_new = (
                mid not in self._original_mods)
            is_core = (
                mid.lower()
                == 'ludeon.rimworld')
            label = self._build_label(
                self.names.get(mid, mid),
                mid, is_core)
            ...
            self.active._model.dataChanged.emit(
                index, index,
                [COLOR_ROLE, TEXT_ROLE,
                 NEW_ROLE,
                 Qt.ItemDataRole.ToolTipRole])
            # pylint: enable=protected-access

    def _mk_avail(self, mid: str, info):
        """Build and add an available-list item
        for *mid*."""
        src = {
            'dlc': '[DLC]',
            'workshop': '[WS]',
        ...
        ver_ok = check_version(
            info, self._game_version())
        is_new = (
            mid not in self._known_mod_ids)
    ...
    def _mk_avail_missing(self, mid: str):
        """Build and add an error item for a mod
        that is in the instance but not on
        disk."""
        label = f"❌ {mid}  [not on disk]"
        tip = (
            "This mod is in the instance "
