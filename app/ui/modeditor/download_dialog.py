...
class DownloadProgressDialog(QDialog):
    """Shows progress while downloading multiple mods via a DownloadQueue."""
    ...
    def __init__(self, parent, download_queue: DownloadQueue,
        ...
        self._mod_names = {wid: name for wid, name in mods_to_download}
    ...
    def _on_job_started(self, _workshop_id: str, title: str) -> None:
        self.current_label.setText(f"Downloading: {title}")
        self.current_bar.setValue(0)
        self._log(f"\n[START] {title}")

    def _on_job_progress(self, _workshop_id: str, percent: int) -> None:
        self.current_bar.setValue(percent)

    def _on_job_finished(self, workshop_id: str, success: bool,
                          msg: str) -> None:
        self._completed += 1
        self.overall_bar.setValue(self._completed)
        self.overall_label.setText(
            f"Overall: {self._completed} / {self._total}")

        name = self._mod_names.get(workshop_id, workshop_id)
    ...
    def _on_job_log(self, _workshop_id: str, line: str) -> None:
        lower = line.lower()
        if any(kw in lower for kw in ('download', 'error', 'success',
                                       'fail', '%')):
            self._log(f"  {line}")
    ...
    def closeEvent(self, event):  # pylint: disable=invalid-name
        """Cancel any pending downloads when the dialog is closed."""
