...
from PyQt6.QtWidgets import (  # pylint: disable=no-name-in-module
    ...
    QToolBar, QStatusBar, QMessageBox, QLabel,
...
from app.core.steamcmd import (
    SteamCMDManager, DownloadQueue,
)
...
from app.core.paths import (
    get_default_data_root, ensure_data_dirs,
    instances_dir, mods_dir,
)
...
class MainWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        self._launched_instance: object | None = None

        self._mod_watcher = ModWatcher(self)
        self._mod_watcher.mods_changed.connect(
            self._on_mods_changed_on_disk)
        self._update_watcher_paths()

        self.grid:   InstanceGridPanel | None = None
        self.detail: InstanceDetailPanel | None = None
        self.sl:     QLabel | None = None
        self.ml:     QLabel | None = None
        self.vl:     QLabel | None = None
        self.gl:     QLabel | None = None
        ...
        if not self.settings.get('rimworld_exe'):
            QTimer.singleShot(
                400, self._auto_detect)
        elif not self.settings.get(
                'offered_import'):
            QTimer.singleShot(
                600, self._offer_import)
        if self._s.update_check_mode == 'auto':
            QTimer.singleShot(
                3000, self._auto_check_updates)
    ...
    def _all_mod_paths(self) -> list[str]:
        ...
        if dr:
            ...
            if p not in paths:
                paths.append(p)
        ws = self.settings.get(
            'steam_workshop_path', '')
        if ws and ws not in paths:
            paths.append(ws)
        exe = self.settings.get(
            'rimworld_exe', '')
        if exe:
            gm = str(Path(exe).parent / 'Mods')
            if gm not in paths:
                paths.append(gm)
        for p in self.settings.get(
                'extra_mod_paths', []):
            if p not in paths:
                paths.append(p)
        return paths
    ...
    def _rescan_mods(self):
        return self.rw.get_installed_mods(
            self._all_mod_paths(),
            force_rescan=True,
            max_age_seconds=0)
    ...
    def _apply_theme(self, force: bool = False):
        ...
        if (self.detail.current_instance
                is not None):
    ...
    def _install_shortcuts(self):
        ...
        ).activated.connect(self._on_new)
        QShortcut(
    ...
    def _build_ui(self):
        ...
        self.grid.instance_selected.connect(
            self._on_select)
        self.grid.instance_double_clicked.connect(
            self._on_launch)
        self.grid.edit_requested.connect(
            self._on_edit)
        self.grid.folder_requested.connect(
            self._open_folder)
        self.grid.export_requested.connect(
            self._on_export)
        self.grid.export_pack_requested.connect(
            self._on_export_pack)
        self.grid.copy_requested.connect(
            self._on_dup)
        self.grid.delete_requested.connect(
            self._on_del)
        self.grid.rename_requested.connect(
            lambda _: self.refresh())
        self.grid.setMinimumWidth(300)
        sp.addWidget(self.grid)

        self.detail = InstanceDetailPanel(self)
        self.detail.launch_requested.connect(
            self._on_launch)
        self.detail.edit_mods_requested.connect(
            self._on_edit_mods)
        self.detail.duplicate_requested.connect(
            self._on_dup)
        self.detail.delete_requested.connect(
            self._on_del)
        self.detail.export_pack_requested.connect(
            self._on_export_pack)
    ...
    def _build_statusbar(self):
        ...
        self.ml = QLabel("")
        self.ml.setStyleSheet(
            "color:#888;font-size:10px;")
        sb.addPermanentWidget(self.ml)
        self.vl = QLabel(
            f"RimWorld {self.rw.version}"
            if self.rw.version else "")
    ...
    def refresh(self, rescan_mods: bool = False):
        """Reload instance list and mod count
        from disk."""
        if rescan_mods:
            self._rescan_mods()
        ...
        installed = self.rw.get_installed_mods(
            self._all_mod_paths())
        self.ml.setText(
            f"{len(installed)} mods installed")

    def _open_child(self, dlg):
        self._child_windows = [
            w for w in self._child_windows
            if w.isVisible()]
    ...
    def _on_launch(self, inst):
        ...
        if inst.mods_configured:
            ...
            inst_exe = (
                ...
                or exe)
            game_mods = (
                Path(inst_exe).parent / 'Mods'
                if inst_exe else None)
    ...
    def _on_dup(self, inst):
        ...
        if ok and name:
            try:
                self.im.duplicate_instance(
                    inst, name)
                self.refresh()
            except Exception as e:  # pylint: disable=broad-exception-caught
                # duplicate can fail from
                # shutil.copytree or
                # Instance.load — heterogeneous
                QMessageBox.critical(
                    self, "Error", str(e))
    ...
    def _on_del(self, inst):
        if QMessageBox.warning(
            ...
            if (self._launched_instance
                    and self._launched_instance
                    is inst):
    ...
    def _on_import_pack(self):
        ...
        dlg = OnyxImportDialog(
            self, Path(path), self.rw, self.im,
    ...
    def _on_logs(self):
        ...
        LogViewerDialog(
            None, self.log_parser, inst).exec()
    ...
    def _auto_check_updates(self):
        ...
        shows badge count in status bar."""
        dr = self.settings.get('data_root', '')
        if not dr:
            return
        ...
        def _run():
            try:
                ...
                installed = (
                    rw.get_installed_mods(
                        extra_mod_paths=paths))
                store = ModTimestampStore(
                    Path(dr))
                ...
                if updates:
                    QTimer.singleShot(
                        ...
                        lambda: self.sl.setText(
                            f"{len(updates)} mod "
                            f"update(s) available"
                            f" — click Check "
                            f"Updates"))
            except Exception:  # pylint: disable=broad-exception-caught
                # daemon thread — network/parse
    ...
    def _apply(self):
        ...
        if dr:
            ensure_data_dirs(Path(dr))
            self.im.instances_root = (
                instances_dir(Path(dr)))
            self.steamcmd.mods_destination = (
                str(mods_dir(Path(dr))))
            self.dl_queue.destination = (
                str(mods_dir(Path(dr))))
        self.launcher.auto_backup = (
            self.settings.get(
                'auto_backup_on_launch', True))
        self.launcher.backup_count = (
            self.settings.get(
                'backup_count', 3))
        cmd = self.settings.get(
            'steamcmd_path', '')
        if cmd:
            self.steamcmd.steamcmd_path = cmd
            self.dl_queue.steamcmd_path = cmd
        self.dl_queue.username = (
            self.settings.get(
                'steamcmd_username', ''))
        self._update_watcher_paths()
        self._init_rw()
    ...
    def _auto_detect(self):
        ...
        if r.found_rimworld:
            msg = (
                "Onyx found your RimWorld:\n\n")
            for line in r.logs:
                msg += f"  • {line}\n"
            msg += "\nApply?"
            if (QMessageBox.question(
                    self, "Auto-Detect", msg)
                    == QMessageBox
                    .StandardButton.Yes):
                self.settings[
                    'rimworld_exe'
    ...
    def _offer_import(self):
        ...
        self._s.update(self.settings)
        existing = (
            self.im.detect_existing_rw_data())
        if not existing:
            return
        msg = (
            f"Found existing RimWorld data:"
            f"\n\n"
        ...
        if (QMessageBox.question(
            ...
            try:
                ...
                # import can fail from
                # shutil.copytree or
                # file I/O — heterogeneous
                QMessageBox.warning(
                    self, "Error", str(e))
    ...
    def closeEvent(self, e):  # pylint: disable=invalid-name
        """Save window geometry and close all child
        windows on exit."""
        self._timer.stop()
